...
with T1:
    ...
    def build_pdf_lab_sheet():
        ...
        story.append(Paragraph("Generated by CognitioCode Sole Proprietorship", styles["Normal"]))
        ...
        med = {c: float(df_real[c].median()) for c in ["Pressure_kPa","Temperature_C","Speed_mms","Needle_um"]}
        ...
        def param_comment(col, value):
            ...
            if col == "Speed_mms":
                return "Favors throughput" if value > med[col] else "Acceptable for resolution"
            if col == "Needle_um":
                return "Fine detail" if value <= med[col] else "Standard"
            if col == "Pressure_kPa":
                return "Within safe median range" if p5[col] <= value <= p95[col] else "Out-of-usual range"
            return ""

        story.append(Paragraph("<b>Recommended Parameters</b>", styles["Heading3"]))
        table_rows = [
            ["Parameter", "Recommended", "5–95% Dataset Range", "Comment"],
            ["Pressure (kPa)", f"{rec['Pressure_kPa']:.1f}", f"{p5['Pressure_kPa']:.0f}–{p95['Pressure_kPa']:.0f}", param_comment("Pressure_kPa", rec["Pressure_kPa"])],
            ["Temperature (°C)", f"{rec['Temperature_C']:.1f}", f"{p5['Temperature_C']:.0f}–{p95['Temperature_C']:.0f}", param_comment("Temperature_C", rec["Temperature_C"])],
            ["Speed (mm/s)", f"{rec['Speed_mms']:.2f}", f"{p5['Speed_mms']:.0f}–{p95['Speed_mms']:.0f}", param_comment("Speed_mms", rec["Speed_mms"])],
            ["Needle (µm)", f"{rec['Needle_um']:.0f}", f"{p5['Needle_um']:.0f}–{p95['Needle_um']:.0f}", param_comment("Needle_um", rec["Needle_um"])],
        ]
        ...
        clog_bad  = metrics["specific_pressure"] > np.percentile(df_real["specific_pressure"], 80)
        ...
        ptab = Table([
            ...
            [f"{metrics['volumetric_flow_rate']:.4f}", f"{metrics['wall_shear_rate']:.0f}", f"{metrics['specific_pressure']:.0f}"],
        ...
        medtab = Table([
            ...
            ["Pressure (kPa)", f"{med['Pressure_kPa']:.1f}", f"{p5['Pressure_kPa']:.0f}–{p95['Pressure_kPa']:.0f}"],
            ["Temperature (°C)", f"{med['Temperature_C']:.1f}", f"{p5['Temperature_C']:.0f}–{p95['Temperature_C']:.0f}"],
            ["Speed (mm/s)", f"{med['Speed_mms']:.1f}", f"{p5['Speed_mms']:.0f}–{p95['Speed_mms']:.0f}"],
            ["Needle (µm)", f"{med['Needle_um']:.0f}", f"{p5['Needle_um']:.0f}–{p95['Needle_um']:.0f}"],
        ...
        story.append(Paragraph(
            "Generated by <b>BioPrint AI</b> — <b>CognitioCode</b>, Istanbul, Turkey",
            styles["Normal"]
        ))
